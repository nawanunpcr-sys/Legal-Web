...
// Gap Analysis ต้องรู้ด้วยว่ามีหลักฐานผูกไว้ที่ระดับฉบับหรือไม่ ไม่ใช่ดูแค่รายข้อ
...
// ═══════════════════════════════════════════════════════════════════════════
// P25 · สายวิเคราะห์ครบวงจร 6 ขั้น + ขั้นที่ 6 สรุปช่องว่าง
// ═══════════════════════════════════════════════════════════════════════════

// ── ฉบับไหนทำขั้นไหนไปแล้ว ─────────────────────────────────────────────────
// อ่านแค่ "มีผลหรือยัง" ต่อ law_id ไม่ดึงเนื้อหามาทั้งก้อน เพราะหน้าตารางต้องการแค่จุดสีเดียว
// ดึงทีเดียวทั้งทะเบียน แทนที่จะยิงต่อแถว — 160 ฉบับ = 4 คำขอ ไม่ใช่ 640
export async function fetchChainMarks() {
  if (!hasSupabase) return { guide: {}, subs: {}, preassess: {}, gap: {} }
  const count = rows => { const m = {}; for (const r of rows || []) m[r.law_id] = (m[r.law_id] || 0) + 1; return m }
  const flag = rows => { const m = {}; for (const r of rows || []) m[r.law_id] = true; return m }
  const [g, s, p, gp] = await Promise.all([
    supabase.from('lg_law_action_guide').select('law_id'),
    supabase.from('lg_sub_requirements').select('law_id'),
    supabase.from('lg_req_ai_suggestion').select('law_id'),
    supabase.from('lg_law_gap').select('law_id'),
  ])
  return { guide: flag(g.data), subs: count(s.data), preassess: count(p.data), gap: flag(gp.data) }
}

// ── งานที่ยังค้างในคิว แยกตามประเภท ────────────────────────────────────────
// คืน { kind: Set(ref) } ให้หน้าจอระบายจุดเป็นสี "กำลังทำ" ได้ตรงตัว
export async function fetchQueuedRefs(kinds = []) {
  if (!hasSupabase || !kinds.length) return {}
  const { data } = await supabase.from('lg_agent_queue')
    .select(`${QUEUE_COL.kind},${QUEUE_COL.ref}`)
    .in(QUEUE_COL.kind, kinds).in('status', ['pending', 'processing'])
  const out = Object.fromEntries(kinds.map(k => [k, new Set()]))
  for (const r of data || []) out[r[QUEUE_COL.kind]]?.add(r[QUEUE_COL.ref])
  return out
}

// ── เข้าคิวงานสายวิเคราะห์ ─────────────────────────────────────────────────
// jobs = [{ kind, ref, label }] จาก chainJobs() · ข้ามรายการที่ยังค้างอยู่แล้ว
// (การ insert แถวคิวไม่ใช่การเรียก AI จึงทำจากหน้าจอได้ — กติกาข้อ 6)
export async function enqueueChain(jobs = []) {
  if (!hasSupabase) throw new Error('ยังไม่ได้ตั้งค่า Supabase')
  if (!jobs.length) return { queued: 0, skipped: 0 }
  const kinds = [...new Set(jobs.map(j => j.kind))]
  const { data: pending } = await supabase.from('lg_agent_queue')
    .select(`${QUEUE_COL.kind},${QUEUE_COL.ref}`)
    .in(QUEUE_COL.kind, kinds).in('status', ['pending', 'processing'])
  const already = new Set((pending || []).map(r => r[QUEUE_COL.kind] + '|' + r[QUEUE_COL.ref]))
  const rows = []
  for (const j of jobs) {
    const key = j.kind + '|' + j.ref
    if (already.has(key)) continue
    already.add(key)
    rows.push({ [QUEUE_COL.kind]: j.kind, [QUEUE_COL.ref]: j.ref,
                [QUEUE_COL.label]: (j.label || '').slice(0, 300), status: 'pending' })
  }
  if (rows.length) {
    const { error } = await supabase.from('lg_agent_queue').insert(rows)
    if (error) throw error
  }
  await logActivity({ action: 'chain_queue', detail: `เข้าคิววิเคราะห์ ${rows.length} งาน (ข้ามที่ค้างอยู่แล้ว ${jobs.length - rows.length} งาน)` })
  return { queued: rows.length, skipped: jobs.length - rows.length }
}

// จำนวนงานค้างแยกตามประเภท — ให้ปุ่มบอกได้ว่าเหลืออะไรอีกบ้าง
export async function chainQueueCounts() {
  if (!hasSupabase) return {}
  const { data } = await supabase.from('lg_agent_queue')
    .select(QUEUE_COL.kind).in('status', ['pending', 'processing'])
  const m = {}
  for (const r of data || []) { const k = r[QUEUE_COL.kind]; m[k] = (m[k] || 0) + 1 }
  return m
}

// สั่งประมวลผลคิวของประเภทเดียว — หน้าจอเป็นผู้ไล่ลำดับตาม CHAIN_ORDER
export async function runChainQueue(kind, max = 50) {
  return callAi('/api/queue-run', { kind, max })
}

// ── ขั้นที่ 6 · สรุปช่องว่างระดับฉบับ ───────────────────────────────────────
export async function fetchLawGap(lawId) {
  if (!hasSupabase) return null
  const { data } = await supabase.from('lg_law_gap').select('*').eq('law_id', lawId).maybeSingle()
  return data || null
}
