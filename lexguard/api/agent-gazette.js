  ...
  try {
    ...
    if (fresh.length) {
      const rows = fresh.map(it => ({
        // เก็บลิงก์ต้นทางของแต่ละรายการที่ agent เจอ (fallback = หน้าแหล่งข้อมูลหลัก)
        source_url: (it.url && /^https?:\/\//i.test(it.url)) ? it.url : SOURCES[0], title: it.title, raw_text: it.summary || '',
