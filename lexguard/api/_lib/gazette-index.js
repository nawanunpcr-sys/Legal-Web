...
export async function findChildAnnouncements(parentLawName, topicNeedle = '', opt = {}){
  ...
  if(topic){
    ...
    // ยังคุมด้วยเพดานผลลัพธ์ กันไว้เผื่อวันหนึ่งชุด discovered โตขึ้นมาก
    for(const n of needles){
      const hits = await searchGazette([n], { ...opt, limit: Math.min(opt.limit || 4, 4), onlyDiscovered: true })
      if(hits.length) return { hits, needle: n, narrowed: false, fromDiscovered: true }
    }
