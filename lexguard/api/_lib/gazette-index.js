...
export async function resolveGazetteLink(lawName){
  ...
  for(const n of gazetteNeedles(name).concat([norm.slice(0, 40)])){
    ...
    for(const h of hits){
      // ── ต้อง "เป็นฉบับนั้น" ไม่ใช่แค่ "เอ่ยถึงฉบับนั้น" ──
      //
      // ชื่อเรื่องในราชกิจจาฯ ของกฎหมายฉบับหนึ่ง จะ *ขึ้นต้น* ด้วยชนิดและชื่อของตัวมันเอง
      // ส่วนเอกสารที่พูดถึงกฎหมายฉบับอื่นจะขึ้นต้นด้วยชนิดของตัวเอง แล้วค่อยเอ่ยถึงทีหลัง
      //
      // เจอจริง: ค้น "พระราชบัญญัติควบคุมอาคาร พ.ศ. 2522" แล้วได้
      //   "คำวินิจฉัยของศาลรัฐธรรมนูญ ที่ 25/2568 เรื่อง พระราชบัญญัติควบคุมอาคาร พ.ศ. 2522
      //    มาตรา 40 … ขัดหรือแย้งต่อรัฐธรรมนูญหรือไม่"
      // ด่านปีไม่จับ เพราะชื่อเรื่องมีปี 2522 ของกฎหมายที่ถูกอ้างอยู่ในนั้นด้วย
      // เปิดไฟล์ดูแล้วเป็นคำวินิจฉัยศาลจริง ๆ — ถ้าปล่อยผ่านจะเอาคำวินิจฉัยไปอ้างเป็นตัวบท
      //
      // เกณฑ์: ชื่อฉบับต้องอยู่ "ต้นชื่อเรื่อง" (ยอมให้มีคำนำหน้าสั้น ๆ ได้ไม่เกิน 12 ตัวอักษร)
      const at = normalizeGazetteTitle(h.title).indexOf(norm.slice(0, 20))
