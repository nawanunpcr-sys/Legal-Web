...
export const PRIMARY_DOMAINS = [
  ...
  'labour.go.th',            // กรมสวัสดิการฯ และสำนักงานพื้นที่ (ครอบคลุม osh. · www. ด้วย)
  // dlpw.go.th ถูกถอดออก 2026-08-21 — อาการเดียวกับ krisdika:
  //   dlpw.go.th · www.dlpw.go.th → ไม่มี A record (DNS ไม่ resolve)
  // ต่างจากโดเมนที่แค่ "ดึงไฟล์ไม่ได้" ตรงที่ลิงก์บนโดเมนนี้เปิดไม่ได้เลยแม้แต่ตัวผู้ใช้เอง
  // ปล่อยไว้ในรายการ = ยอมให้คำตอบติดที่มาเป็นลิงก์ตาย ซึ่งแย่กว่าการบอกว่าหาไม่เจอ
  // งานของกรมสวัสดิการฯ ยังครอบคลุมด้วย labour.go.th (osh.labour.go.th ใช้ได้จริง)
  ...
  // มหาดไทย — เพิ่ม 2026-08-22 · law.moi.go.th เสิร์ฟ "ฉบับรวมของสำนักงานคณะกรรมการกฤษฎีกา"
  //   ยิงจริง law.moi.go.th/105.pdf = พ.ร.บ.ควบคุมอาคาร พ.ศ. 2522 ฉบับเต็ม
  //   147,616 อักขระ · ไทย 90% · ผ่าน pdfTextUsable · มีมาตรา 8 (บทให้อำนาจออกกฎกระทรวง)
  //   ทางหนีไฟ บันไดหนีไฟ ครบ — เป็นตัวบทชนิดเดียวกับที่ krisdika.go.th เคยให้ก่อนโดเมนตาย
  //   และมหาดไทยคือกระทรวงที่ออกกฎกระทรวงควบคุมอาคารทุกฉบับ (ฉบับที่ 39 · 55 · 63)
  'moi.go.th',
...
const DEAD_SOURCES = [
  ...
  // ── เพิ่ม 2026-08-22 พร้อมกับชั้น .go.th ──
  // ก่อนหน้านี้สองโดเมนนี้ถูกกันโดยปริยาย เพราะไม่ได้อยู่ใน TRUSTED_DOMAINS
  // พอเปิดชั้น .go.th ให้ผ่านโดยอัตโนมัติ มันเลยกลับเข้ามาผ่านด่านโดเมนได้อีก
  // ทั้งที่ยิงจริงวันนี้ยังต่อไม่ติดทั้งคู่ (http=000) — ต้องกันตรงนี้แทน
  // ไม่งั้นจะเสียคำขอไปกับการดึงของที่ดึงไม่ได้ แล้วผู้ใช้เห็นเหตุผลผิดว่า "ไม่พบตัวบท"
  { re: /^https?:\/\/(?:www\.)?krisdika\.go\.th(?:[:/]|$)/i,
    why: 'apex ไม่มี DNS record และ www. ใช้ SSL self-signed — fetch() ต่อไม่ติด (ยืนยันซ้ำ 2026-08-22)' },
  { re: /^https?:\/\/(?:www\.)?dlpw\.go\.th(?:[:/]|$)/i,
    why: 'ไม่มี A record ทั้ง apex และ www. — งานของกรมสวัสดิการฯ เปิดได้ที่ osh.labour.go.th แทน' },
]
...
// ── ชั้นที่ 3 · หน่วยงานราชการ/องค์กรวิชาชีพไทย ที่ยังไม่ได้อยู่ในรายชื่อ ─────
//
// ปัญหาของรายชื่อตายตัว: หน่วยงานไทยมีหลายร้อยแห่ง และตัวบทฉบับหนึ่งอาจถูกเก็บไว้
// ที่เว็บของหน่วยงานที่เราไม่ได้นึกถึงตอนเขียนรายชื่อ · ผลคือระบบตอบ "ไม่พบ"
// ทั้งที่ตัวบทเปิดได้อยู่ และเหตุผลที่ปฏิเสธคือ "เราไม่ได้ใส่ชื่อโดเมนนั้นไว้" ซึ่งไม่ใช่เหตุผลเรื่องคุณภาพ
//
// `.go.th` ออกให้เฉพาะหน่วยงานรัฐไทย · `.or.th` เฉพาะองค์กรไม่แสวงกำไร/สมาคมวิชาชีพที่จดทะเบียน
// ทั้งคู่ตรวจได้จาก URL ตรง ๆ ไม่ต้องมีใครมาตัดสินว่า "น่าเชื่อถือไหม"
// ต่างจาก `.com` ที่ใครก็จดได้ — เกณฑ์นี้จึงเป็นกฎ ไม่ใช่ความเห็น
//
// ⚠ ชั้นนี้ "ตอบได้" แต่ไม่ใช่ "เชื่อได้เท่าต้นฉบับ"
// เว็บหน่วยงานบางแห่งเก็บฉบับก่อนแก้ไขไว้โดยไม่บอก และเทศบาล/อบต. มักคัดของกลางมาวาง
// คำตอบจากชั้นนี้จึงติดป้ายเตือนเหมือนชั้น 2 ทุกครั้ง · ด่านหลักฐานยังบังคับครบ:
// ต้องดึงไฟล์มาอ่านจริงและคัด source_excerpt จากตัวบทได้ ไม่ใช่เชื่อ snippet
const THAI_OFFICIAL_RE = /(?:^|\.)(?:go|or)\.th$/i

export function isThaiOfficialHost(u){
  try{ return THAI_OFFICIAL_RE.test(new URL(u).hostname.toLowerCase()) }catch{ return false }
}
...
export function hostAllowed(u){
  if(deadSource(u)) return false
  return inList(u, TRUSTED_DOMAINS) || isThaiOfficialHost(u)
}
...
// ชั้นของแหล่ง — UI กับ note ใช้บอกผู้ใช้ว่าคำตอบนี้เชื่อได้แค่ไหน
//   primary   ต้นทางที่เราคัดไว้เอง (ราชกิจจาฯ · เจ้าของกฎหมาย)
//   secondary ฉบับรวมขององค์กรวิชาชีพที่เราคัดไว้ (ASA · SHAWPAT)
//   official  .go.th / .or.th อื่น ๆ ที่ผ่านกฎ แต่เราไม่ได้ตรวจคุณภาพรายเว็บ
export function sourceTier(u){
  if(!u || deadSource(u)) return 'none'
  if(inList(u, PRIMARY_DOMAINS)) return 'primary'
  if(inList(u, SECONDARY_DOMAINS)) return 'secondary'
  if(isThaiOfficialHost(u)) return 'official'
  return 'none'
}
...
// 45 วิเดิมสูงเกินไปเมื่อเทียบกับของจริง: ไฟล์ที่ดึงได้ใช้เวลา 1.9–2.6 วิ (ยืนยัน 2026-08-21
// กับราชกิจจาฯ 3 ไฟล์ 26–85 KB) ส่วนโฮสต์ที่ body ไม่ยอมจบ (osh.labour.go.th · www.pcd.go.th
// ส่ง chunked แล้วค้างคาสาย) จะกินจนครบเพดานทุกครั้งโดยไม่ได้อะไรกลับมา
// หนึ่งที่อยู่แบบนั้น = 45 วิจากงบ 255 วิ · เจอสองที่อยู่ก็หายไปหนึ่งในสามของงบทั้งรอบ
// 20 วิยังเผื่อไว้เกือบ 10 เท่าของเวลาที่ไฟล์จริงใช้ · ถ้าวันหนึ่งไฟล์ใหญ่จริงถูกตัดทิ้ง
// ให้ขยับขึ้นทีละ 5 วิ อย่ากลับไป 45
const PDF_FETCH_TIMEOUT = 20_000
...
- legal.labour.go.th เปิดไม่ได้ (ใบรับรอง SSL ไม่ผ่าน) และ dlpw.go.th ไม่มีอยู่แล้ว (DNS ไม่ resolve)
  กฎหมายแรงงาน/ความปลอดภัย ให้ใช้ ratchakitcha.soc.go.th/documents/<เลข>.pdf เป็นอันดับแรก
  รองลงมาคือ tosh.or.th หรือ osh.labour.go.th
...
แหล่งที่ใช้เป็น source_url ได้ เรียงตามความน่าเชื่อถือ:
1. ราชกิจจานุเบกษา — ต้นฉบับ ใช้ก่อนเสมอเมื่อหาเจอ
2. เว็บของหน่วยงานที่ออกกฎหมายฉบับนั้นเอง (กรมอนามัย · กรมสวัสดิการฯ · กรมโยธาฯ ฯลฯ)
3. ฉบับรวมขององค์กรวิชาชีพ (asa.or.th · shawpat.or.th) — แม่นกว่าสำหรับกฎหมายที่ถูกแก้หลายรอบ
4. **เว็บหน่วยงานราชการไทยอื่นที่ลงท้าย .go.th หรือองค์กรวิชาชีพที่ลงท้าย .or.th**
   ใช้ได้ถ้าหน้านั้นมีตัวบทจริงให้อ่าน — ไม่ต้องอยู่ในรายชื่อที่ระบบเคยกำหนดไว้

ที่อยู่ที่ **ห้ามใช้เป็น source_url เด็ดขาด** ไม่ว่าข้อมูลจะดูถูกต้องแค่ไหน:
- บล็อก บทความ สรุปของสำนักงานที่ปรึกษา เว็บข่าว เว็บ .com/.net ทุกชนิด
- เว็บรวมกฎหมายของเอกชนที่ไม่ใช่ .go.th / .or.th
ค้นเจอเนื้อหาจากแหล่งเหล่านี้แล้วมั่นใจว่าถูก **ห้ามเอามาตอบ** ให้ใช้เป็นเบาะแสหาชื่อฉบับ
แล้วไปเปิดตัวบทจากแหล่ง 1–4 แทน · เปิดไม่ได้ให้ตอบ not_answered พร้อมบอกว่าเบาะแสชี้ไปที่ฉบับใด
เหตุผล: ทะเบียนนี้ใช้ตรวจ ISO ผู้ตรวจถามว่าอ้างจากอะไร แล้วคำตอบเป็นบล็อกคือใช้ไม่ได้ทันที`

// ── ไม่ล็อก allowed_domains อีกแล้ว (2026-08-22) ─────────────────────────────
// เดิมล็อกไว้ที่ TRUSTED_DOMAINS ซึ่งกันขยะได้จริง แต่กันของดีไปด้วย:
// ตัวบทฉบับหนึ่งอาจถูกเก็บไว้ที่เว็บหน่วยงานที่เราไม่ได้นึกถึงตอนเขียนรายชื่อ
// โมเดลจึง "ค้นไม่เจอ" ตั้งแต่ต้น ไม่ใช่ "เจอแล้วถูกตัด" — และเราไม่มีทางรู้ว่าพลาดอะไรไป
//
// ตอนนี้ปล่อยให้ค้นกว้าง แล้วบังคับที่ "ผลลัพธ์" แทนด้วย hostAllowed() ในด่าน (ก)
// ซึ่งรับเฉพาะ .go.th / .or.th (+ รายชื่อหลัก) และยังต้องผ่านด่านหลักฐานครบทุกข้อ
// การกรองที่ผลลัพธ์แม่นกว่าการกรองที่คำค้น เพราะเห็นของจริงแล้วค่อยตัดสิน
